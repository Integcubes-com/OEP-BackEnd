...
namespace ActionTrakingSystem.Model
{
    ...
    public class UpdatedRegionsController : BaseAPIController
    {
        private readonly DAL _context;
        public UpdatedRegionsController(DAL context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<IEnumerable<Regions>>> GetRegions()
        {
            try
            {
                var regions = await (from r in _context.Regions2.Where(a => a.isDeleted == 0)
        ...
        public async Task<IActionResult> DeleteRegion(RegionsDto reg)
        {
            try
            {
                Regions2 regions = await (from r in _context.Regions2.Where(a => a.regionId == reg.regionId)
        ...
        public async Task<IActionResult> saveRegion(RegionUserDto reg)
        {
            try
            {
                ...
                {
                    Regions2 region = await (from r in _context.Regions2.Where(a => a.regionId == reg.data.region.regionId)
                                            select r).FirstOrDefaultAsync();
                    region.title = reg.data.region.title;
                    region.TILsSummary = reg.data.region.TILsSummary;
                    region.insuranceSummary = reg.data.region.insuranceSummary;
