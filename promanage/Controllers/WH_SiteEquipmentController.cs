    ...
    public class WH_SiteEquipmentController : BaseAPIController
    {
        ...
        public async Task<IActionResult> GetSiteEquipment(getSiteEquiomentDto reg)
        {
            try
            {
                ...
                var siteEquipment = await (from se in _context.SiteEquipment.Where(a => a.isDeleted == 0 && (reg.filterObj.modelId == -1 || a.fleetEquipmentId == reg.filterObj.modelId) && (reg.filterObj.siteId == -1 || a.siteId == reg.filterObj.siteId))
